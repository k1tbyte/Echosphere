...
const variants: [string, ReactElement][] = [
    [ "Info", <SvgIcon icon={EIcon.InfoCircleOutline} className="fill-foreground" size={26}/> ],
    [ "Warning", <SvgIcon icon={EIcon.AlertCircleOutline} className="fill-yellow-300" size={26}/> ],
    [ "Error", <SvgIcon icon={EIcon.CloseCircleOutline} className="fill-destructive" size={26}/> ],
    [ "Success", <SvgIcon icon={EIcon.CheckCircleOutline} className="fill-green-500" size={26}/> ],
]
...
const Toast: FC<IToastProps> = React.memo(
    ...
    const [ title, icon ] = variants[variant]
        ...
        <motion.div onMouseEnter={() => clearTimeout(timer)}
                    ...
                    className="w-full max-w-96  p-3.5 bg-background/75 border border-border rounded-md pointer-events-auto drop-shadow-md">
            <div className="flex items-start justify-between">
                <div className="flex gap-2.5 items-center">
                    {icon}
                    <span className="font-medium text-foreground-accent">{title}</span>
                </div>
                {canClose && <SvgIcon className="fill-foreground hover:fill-foreground-accent transition-colors cursor-pointer"
                                      icon={EIcon.Close} role="button"
                    // @ts-ignore
                                      onClick={(e) => {
                                          e.stopPropagation();
                                          onClose();}}
